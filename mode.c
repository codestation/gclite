...
{
	int i;
	u32 *array = (u32 *)*(u32 *)((*(u32 *)(text_addr_game+PATCHES->struct_addr))+((u32)folder << 2));
	char *title = (char *)array[68/4];
	
	Category *p = GetNextCategory(NULL);

	for (i = PATCHES->index; p; i++)
	{
		char *name = &p->name;
...
int scePafAddGameItemsPatched(void *unk, int count)
{
	return scePafAddGameItems(unk, CountCategories());
}
...
ToggleCategoryPatch ToggleCategoryPatches_620[] =
{
	...
	{ 0x00019940, 0x00000000 },
	{ 0x00019A18, 0x00000000 },
	{ 0x00019AE8, 0x00000000 },
	{ 0x00019B94, 0x10000006 },
};
